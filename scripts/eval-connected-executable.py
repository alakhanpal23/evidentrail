...
WORKSPACE_FILES = {
    "db-pool-zero": ("api/pool.conf", "test_db_pool"),
    "migration-drift": ("worker/migrations.conf", "test_migration"),
    "upstream-timeout": ("gateway/upstream.conf", "test_upstream"),
}

REPAIR_FIXTURE = Path(__file__).resolve().parents[1] / "fixtures/connected-repair-app"


def service_test(workspace, scenario):
    test_name = WORKSPACE_FILES[scenario][1]
    result = subprocess.run(
        [sys.executable, "-m", "unittest", f"test_service.RepairTests.{test_name}"],
        cwd=workspace,
        capture_output=True,
        timeout=10,
    )
...
def run_workspace_agent(model, helper, directory):
    """Run an isolated file edit through a service test, with a no-log control."""
    ...
    for scenario, question in QUESTIONS.items():
        for method in ("no_logs", "first_id", "severity", "recent", "model"):
            ...
            with tempfile.TemporaryDirectory(prefix="evidentrail-agent-workspace-") as scratch:
                ...
                shutil.copytree(REPAIR_FIXTURE, workspace, dirs_exist_ok=True)
                if service_test(workspace, scenario):
                    raise AssertionError("the unedited service must fail its target test")
                started = time.monotonic()
                ...
                if not abstain and valid_line:
                    ...
                    service_passed = service_test(workspace, scenario)
                    if chosen == WORKSPACE_FILES[scenario][0]:
                        ...
                        helper_verified = result.returncode == 0 and result.stdout == b"verification=passed\n"
                verified = helper_verified and service_passed
                elapsed_ms = int((time.monotonic() - started) * 1000)
                hits[method] = hits.get(method, 0) + int(verified)
                print("EXEC_CONNECTED_WORKSPACE", json.dumps({
                    ...
                    "patch_verified": verified, "service_test_passed": service_passed,
                    "elapsed_ms": elapsed_ms,
                }, sort_keys=True), flush=True)
...
def main():
    ...
    parser.add_argument("--workspace-agent", action="store_true", help="run a bounded file-read/edit agent, executable service tests, and a no-logs control")
