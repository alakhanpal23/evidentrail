...
The August 30 pre-Keychain workspace gate registered 827 tests: 823 passed and four explicit opt-in pinned-external tests were ignored. The current revision adds Keychain, restart, and Linux compatibility contracts; final counts and live CI status belong in the release handoff rather than this frozen paragraph. The provisioned Keychain test is intentionally opt-in because an unsigned Cargo test host receives `errSecMissingEntitlement`.
