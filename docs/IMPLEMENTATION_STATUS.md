...
| Optional hosted evidence ranking | Provider-neutral application interface; passthrough/mandatory-feasibility-before-egress ordering; at most 32 intact optional blocks and 40 KiB escaped payload using reversible byte escaping; strict complete-permutation response validation; model-order, reciprocal-rank-fusion, and bounded-fourth-affinity consumers; one no-retry OpenAI Responses adapter with strict structured output, no tools/history, `store:false`, 800 ms deadline, 64-KiB completed/singular/non-refusal provider-envelope validation, native credential lookup, configuration-bound contentless diagnostics on CLI/MCP, kill switch, internal deterministic-output shadow mode, CLI `--llm-rank`, and MCP `ranking_mode` | Default remains deterministic and offline. Focused tests prove byte-identical fallback, no call for passthrough/`needs_more`/oversized egress, malformed/duplicate/foreign/missing-ID rejection, mandatory-authority isolation, bounded budget accounting, arbitrary-response parser closure, a loopback live-adapter wire contract, shadow output identity, and contentless diagnostics. The OpenAI adapter is pinned to the current beta candidate; no multi-vendor winner or production-admission claim exists until the frozen live benchmark and release gates are run. |
