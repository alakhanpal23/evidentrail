...
| --- | --- | --- |
| M1 performance qualification | Lazy 10K/100K/1M local/CloudWatch/Kubernetes fixtures; frozen semantic baselines; balanced paired order; 30-pair throughput and 200-pair latency protocols; paired BCa one-sided bounds with 20,000 resamples; RSS/storage/exactness/expansion fields; dedicated-host, thermal, authority-class, and clean-boot eligibility gates; real memory/V2-durable product bindings; reference-host capture and cold-arm staging scripts | Statistical and real-arm conformance smoke tests pass. No production performance claim exists: `ProcessKeyAuthorityV2` is rejected as conformance-only, and qualification remains blocked on a Keychain-backed V2 authority. Checked-in environment/report manifests remain explicitly unmeasured. |
...
| Security hygiene | Contentless Debug/Error canaries across implemented types; hostile bytes cannot inject Log Brief structure; fixed-width XChaCha20-Poly1305 snapshot frames/manifest envelope, AAD, commitments, one zeroizing result DEK, nonce registry, domain-separated wrapping/seal keys, fixed-width AEAD key objects, canonical Creating/Sealed key records, a state-oriented bounded key-provider contract, untrusted fixed-width cleanup hints, segment catalogs, catalog-bound EventId expansion indexes, receipt-ordered source-outcome tables, a bounded self-restoring acquisition-completion record, a canonical ciphertext-only sealed-result bundle, a Unix ciphertext-only filesystem publication substrate, a provider-generic authenticated post-render retention adapter, and an encrypted displayed-alias manifest | The V2 durable repository now uses a four-state external authority record, persisted pending nonce reservations, authority-allocated repository-global publication generations, encrypted exact-retry batch/acknowledgement journals, and a 16,384-entry sharded authenticated index (62 shards at the one-million-event cap) whose point reader decrypts only requested shards. Descriptor-relative no-follow I/O, strict ownership/mode/link validation, create-only publication, `F_FULLFSYNC` on macOS, reconciliation, read leases, authority-first destruction, and orphan quarantine are covered by focused contracts. Product migration seals every ledger event before deleting plaintext and publishes aliases only after the repository gate. Production Keychain authority, authenticated expected-context/result catalogs, configured startup, and dedicated-host sudden-power-loss/performance qualification remain open. |
...
The August 30 all-target/all-feature workspace gate registered 827 tests: 823 passed and four explicit opt-in pinned-external tests were ignored by the ordinary run. Workspace-wide warnings-as-errors Clippy, strict rustdoc, feature-partition, schema-drift, formatting, and diff checks also passed on Rust 1.88. The four opt-in external smokes remain unavailable because their exact pinned `/opt/evidentrail-bench/legacy-drain` checkout is absent on this host; no current external-rerun claim is made, and the August 25 pinned evidence remains historical. Linux and MSRV 1.85 execution remain CI-owned because those local toolchains are not installed. Green tests do not override the explicit release blockers above; re-run the root README commands because counts change as slices land.
