...
The August 25 clean all-target/all-feature workspace gate registered 817 tests: 813 passed and four explicit opt-in pinned-external tests were ignored by the ordinary run. Those four external tests were then run separately against the exact clean pinned checkout and passed. Workspace-wide warnings-as-errors Clippy, formatting, and diff checks also passed. Green tests do not override the explicit release blockers above; re-run the root README commands because counts change as slices land.
